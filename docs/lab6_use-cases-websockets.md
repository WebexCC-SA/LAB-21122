...
1. Navigate to `06-usecases/01_feedback_websocket.py` and review the code.

   These are the key points to understand how each requirement has been met:

   1. **Permitted User Access Only**
      The **`is_allowed_sender()`** handler function verifies the user's email against a predefined allow-list before executing feedback commands.
   2. **Org-wide Feedback Card Distribution**
      The **`/feedback`** command handler sends an Adaptive Card containing a feedback form to every user in the Webex organization.
   3. **Adaptive Card Feedback Input**
      The Adaptive Card created by the bot includes a multi-line **Input.Text** element specifically for feedback entry.
   4. **Collect Responses, Notify Sender**
      The **`on_attachment_action()`** handler extracts the feedback from the submitted card, forwards it to the designated email, and sends a confirmation or error message back to the submitting user.
...
3. Execute the code with the following command and let it run:
   ...
   * python 01_feedback_websocket.py
...
1. Navigate to `06-usecases/02_device_websocket.py` and review the code.
   ...
   2. **Adaptive Card presentation**
      The **`on_message()`** handler displays an Adaptive Card with MAC input and a phone model dropdown when the user requests provisioning.
   3. **Device provisioning**
      The **`on_attachment_action()`** handler extracts card data and POSTs it to the `/v1/devices` API for provisioning.
   4. **Provisioning result notification**
      **`on_attachment_action()`** returns confirmation messages (success, duplicate, or error) based on the API's response status.

2. Execute the code with the following command and let it run:

   * python 02_device_websocket.py
